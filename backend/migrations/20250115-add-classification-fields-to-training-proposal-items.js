...
const addClassificationFieldsToTrainingProposalItems = async () => {
  try {
    console.log('=== MIGRATION: Adding classification fields to training_proposal_items ===');
    ...
    const [results] = await sequelize.query(`
      ...
      AND TABLE_NAME = 'training_proposal_items' 
    ...
    if (!existingColumns.includes('Jenis')) {
      ...
      await sequelize.query(`
        ALTER TABLE training_proposal_items 
    ...
    console.log('All classification fields have been added to training_proposal_items table');
...
// Run migration
addClassificationFieldsToTrainingProposalItems()
  .then(() => {
    console.log('\nMigration finished');
    process.exit(0);
  })
